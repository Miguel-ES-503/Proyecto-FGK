 ...
 $resultol =  $TotalTalleresAlumno +  $TotalExternaTallerAlumno + $TotalReunionAlumno;
  ...
  if ($consulta15->rowCount() >=0)
  {
   ...
   if ($EstadoBeca == "Reprobado") {
   	$EstadoBecaFinal = '<p style="color: red;">Reprobado</p>';
   }
   else
   {
   	$EstadoBecaFinal = '<p style="color: green;">Aprobado</p>';
   }
   ...
   if($Promedio >= 75)
   {
   	$PorcentajeColor = '<p style="color: green; text-align: center;">'.round($Promedio,2).' % </p>';
   }
   else if($Promedio >= 50 && $Promedio < 74  )
   {
   	$PorcentajeColor = '<p style="color: orange; text-align: center;">'.round($Promedio,2).' % </p>';
   }
   else
   {
   	$PorcentajeColor = '<p style="color: red; text-align: center;">'.round($Promedio,2).' % </p>';
   }
   ...
   else
   {
		...
		<td><b>".$fila['Status']."</b></td>
		<td><b>".$fila['StatusActual']."</b></td>
		<td>".$TotalInterno."</td>
		<td>".$TotalExternaTallerAlumno."/".$TotalEmpresasExterna."</td>
		<td>".$TotalReunionAlumno."/".$TotalReuniones."</td>
		<td><b>".$resultol."</b></td>
		<td>".$HorasSociales."</td>
		<td><b>".$PorcentajeColor ." </b></td>
		<td><b>".$EstadoBecaFinal."</b></td>
