...
$consulta=$pdo->prepare("SELECT `ID_Reunion` , `Titulo` , `Fecha` , e.Nombre AS 'uni' , `ID_Ciclo`, `Estado` , `ID_Sede`  FROM reuniones r INNER JOIN empresas e on r.ID_Empresa = e.ID_Empresa WHERE  Estado = 'Finalizado' AND YEAR(`Fecha`) = ? AND MONTH (Fecha) = ? AND `ID_Sede` = ? ");
