   ...
   <br>
    ...
    <div class="collapse navbar-collapse" id="basicExampleNav">
        ...
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="ReporteCiclo.php">Ciclo</a>
            </li>
...
<br>
 ...
 <?php 
        ...
        $consulta1=$pdo->prepare("SELECT  COUNT(DISTINCT(I.ID_Alumno)) as 'Total'
            ...
            WHERE T.ID_Sede = ? AND I.Asistencia = 'Asistio' AND T.ID_Ciclo = ? AND Date_Format(T.Fecha,'%Y') = ?");
        ...
        $consulta3=$pdo->prepare("SELECT COUNT(E.ID_Alumno) as 'Total'
            ...
            where E.Rating = '0'   AND T.ID_Ciclo = ? AND T.ID_Sede = ? AND Date_Format(T.Fecha,'%Y') = ?");

        $consulta4=$pdo->prepare("SELECT COUNT(E.ID_Alumno) as 'Total'
            from evaluaciontalleres E
            INNER JOIN talleres T 
            ON E.ID_Taller = T.ID_Taller
            where E.Rating = '50' AND T.ID_Ciclo = ? AND T.ID_Sede = ? AND Date_Format(T.Fecha,'%Y') = ? ");

        $consulta5=$pdo->prepare("SELECT COUNT(E.ID_Alumno) as 'Total'
            from evaluaciontalleres E
            INNER JOIN talleres T 
            ON E.ID_Taller = T.ID_Taller
            where E.Rating = '100' AND T.ID_Ciclo = ? AND T.ID_Sede = ? AND Date_Format(T.Fecha,'%Y') = ?");
        $consulta6=$pdo->prepare("SELECT COUNT(E.ID_Alumno) as 'Total' 
            from evaluaciontalleres E
            INNER JOIN talleres T 
            ON E.ID_Taller = T.ID_Taller
            where   T.ID_Ciclo = ? AND T.ID_Sede = ? AND Date_Format(T.Fecha,'%Y') = ? ");
         $consulta7=$pdo->prepare("SELECT  COUNT(DISTINCT(I.ID_Alumno)) as 'TotalHOM' 
            ...
            WHERE T.ID_Sede = ? AND I.Asistencia = 'Asistio' AND T.ID_Ciclo = ? AND Date_Format(T.
        ...
        $consulta9=$pdo->prepare("SELECT COUNT(E.ID_Alumno) as 'Total'
            ...
            INNER JOIN talleres T 
            ON E.ID_Taller = T.ID_Taller
            where E.Rating = '25'   AND T.ID_Ciclo = ? AND T.ID_Sede = ? AND Date_Format(T.Fecha,'%Y') = ?");
        $consulta10=$pdo->prepare("SELECT COUNT(E.ID_Alumno) as 'Total'
            from evaluaciontalleres E
            INNER JOIN talleres T 
            ON E.ID_Taller = T.ID_Taller
            where E.Rating = '75'   AND T.ID_Ciclo = ? AND T.ID_Sede = ? AND Date_Format(T.Fecha,'%Y') = ?");


        $consulta1->execute(array($FSedes,$FCiclos,$FYear));//Asistencia real
        $consulta2->execute(array($FSedes,$FCiclos,$FYear));//Asistencoa de impacto
        $consulta3->execute(array($FCiclos,$FSedes,$FYear));//Todal con Rating = 0
        $consulta4->execute(array($FCiclos,$FSedes,$FYear));//Todal con Rating = 50
        $consulta5->execute(array($FCiclos,$FSedes,$FYear));//Todal con Rating = 100
        $consulta6->execute(array($FCiclos,$FSedes,$FYear));//Todal de evaluaciones
        $consulta7->execute(array($FSedes,$FCiclos,$FYear));//Total real de hombres que asistieron
        $consulta8->execute(array($FSedes,$FCiclos,$FYear));//Total real de mujeres que asistieron
        $consulta9->execute(array($FCiclos,$FSedes,$FYear));//Todal con Rating = 25
        $consulta10->execute(array($FCiclos,$FSedes,$FYear));//Todal con Rating = 75
...
?>
        ...
        <div class="row" style="margin: 3.5%;">
            <div class="col-sm" id="GEvaluaciones">
                <!--Inicio la seccion de grafico Evaluaciones-->
                <div class="GraficosPanel">
                    <!--Load the AJAX API-->
                    <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
                    <script type="text/javascript">
