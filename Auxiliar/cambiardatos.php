...
<input type="number" class="form-control"  name="data1" value='<?php echo $row['CantidadModulos']; ?>' max='6' placeholder="Módulos">
</div>
<div class="form-group"  class="text-dark">
<label for="exampleInputPassword1"  class="text-dark float-left">Talleres</label>
<input type="number" class="form-control" id="password" min = "0" name='data2' value='<?php echo $row['TotalTalleres']; ?>' placeholder="Talleres">
</div>
...
<label for="fName"  class="text-dark float-left" >Para finalizar el proceso, ingrese la contraseña de su usuario.</label>
...
<button type="submit" name="data4" class="btn btn-success" value='<?php echo $row['ID_Alumno']; ?>' />Actualizar</button>
</fieldset>
</form>
</div>
<?php }?>
...
if (isset($IdAlumno)) {
    ...
    $sql = "UPDATE alumnos SET CantidadModulos=?, TotalTalleres=? WHERE ID_Alumno=?";
    $pdo->prepare($sql)->execute([$modulos, $talleres, $IdAlumno]);
