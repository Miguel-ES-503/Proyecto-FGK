...
$stmt = $pdo->prepare("SELECT alumnos.Nombre AS alumno , alumnos.Class, datos_modulos.id_alumno , datos_modulos.id, datos_modulos.id_modulo , empresas.Nombre, datos_modulos.estado FROM datos_modulos LEFT JOIN alumnos ON datos_modulos.id_alumno =  alumnos.ID_Alumno LEFT JOIN empresas ON empresas.ID_Empresa = alumnos.ID_Empresa WHERE datos_modulos.id_modulo = ?  ");
...
$pdf->Cell(40);
 ...
 $pdf->SetFont('Arial','B',17);
 $pdf->Cell(112);
 $pdf->Cell(140,6,utf8_decode('Lista de alumnos inscritos Modulo A1'), 0, 1);
 $pdf->Ln(10);
 ...
 $pdf->Cell(0,6,'',1,1,'L');
    ...
    $stmt->execute(array('MOD50000005'));  
