...
$consulta=$pdo->prepare("SELECT id_solicitud, a.Nombre nombreAlumno, a.Class AS clase, a.ID_Sede AS sede, a.ID_Empresa AS Universidad, sd.estado AS estadoSoli FROM solicituddesinscribir sd JOIN alumnos a on sd.id_alumno=a.ID_Alumno INNER JOIN talleres t ON sd.id_taller=t.ID_Taller WHERE sd.estado='En espera' AND a.SedeAsistencia= ? OR a.SedeAsistencia= ? ");
...
if ($consulta->rowCount()>=1)
{
	while ($fila=$consulta->fetch()) {
		...
		<td><a href='DetallesDesincribirTaller.php?id=".$fila['id_solicitud']."' class='btn btn-success'>Ver Detalles </a> </td>
