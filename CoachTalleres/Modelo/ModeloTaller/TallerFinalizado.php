...
$consulta=$pdo->prepare("SELECT  ID_Taller ,Titulo , Fecha , Hora, F.Nombre AS 'Tipo' ,ID_Sede , ID_Ciclo, E.Nombre AS Empresa ,Estado FROM talleres T INNER JOIN formatotalleres F on T.ID_Formato = F.ID_Formato LEFT JOIN empresas E on T.ID_Empresa = E.ID_Empresa WHERE ID_Sede = ?  AND Estado = 'Finalizado' AND YEAR(`Fecha`) = ? AND MONTH (Fecha) = ? ");
$consulta->execute(array($LugarFT,$YearActual,$MesActual));
