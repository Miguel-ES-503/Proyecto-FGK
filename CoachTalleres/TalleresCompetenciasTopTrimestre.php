    ...
    <div class="collapse navbar-collapse" id="basicExampleNav">
        ...
        <ul class="navbar-nav mr-auto">
            <li class="nav-item">
                ...
                echo " <a href='ReporteCompetenciasTrimestre.php'><button type='button' class='btn btn-light'>Regresar</button></a>";
...
<div class="card" style="margin: 3%;">
    <div class="card-header">
        <p style="font-size: 25px; color: black;"> Lista de talleres <?php  //echo $ubicacion ?></p>
    </div>
    <div class="card-body">
        ...
        <table id="example" class="table table-responsive">
            ...
            <tbody>
                ...
                $consulta=$pdo->prepare("   SELECT A.Titulo as 'Titulo', A.Fecha as 'Fecha',S.Nombre as 'Sede',F.Nombre as 'Tipo'
                    ...
                    WHERE A.ID_Sede = ? AND ( Month(A.Fecha) >= ? AND Month(A.Fecha) <= ?) AND Date_Format(A.Fecha,'%Y')= ? AND T.IDComptenecia = ?
                    ");
                $Year = $_GET['id'];
                $Sede = $_GET['id2'];
                $T1 = $_GET['id3'];
                $T2 = $_GET['id4'];
                $IdCompetencia = $_GET['id5'];
                setlocale(LC_TIME, 'es_SV.UTF-8');
                echo $Sede.$T1.$T2.$IdCompetencia;
                $consulta->execute(array($Sede,$T1,$T2,$Year,$IdCompetencia));
